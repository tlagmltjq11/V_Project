...
public class Player_MOVE : FSMSingleton<Player_MOVE>, IFSMState<Player_StateManager>
{
    ...
    public void Execute(Player_StateManager e)
    {
        ...
        if (e.m_isCrouching)
        {
            ...
            e.m_charCon.height = Mathf.Lerp(e.m_charCon.height, e.m_crouchHeight, e.m_crouchMotionSpeed * Time.deltaTime);
            if (e.m_charCon.height - 0.05f <= e.m_crouchHeight)
            {
                e.m_charCon.height = e.m_crouchHeight;
            }

            e.m_charCon.center = e.m_crouchCenter;
        }
        else //서있는 상태
        {
            e.m_animator.SetBool("ISCROUCH", false);
            e.m_charCon.height = Mathf.Lerp(e.m_charCon.height, e.m_standHeight, e.m_crouchMotionSpeed * Time.deltaTime);
            if (e.m_charCon.height + 0.05f >= e.m_standHeight)
            {
                e.m_charCon.height = e.m_standHeight;
            }

            
            e.m_charCon.center = e.m_standCenter;
        }
        */
        ...
        if (!e.m_isGrounded && e.m_velocity.y < 0f && !e.m_isCrouching)
        {
